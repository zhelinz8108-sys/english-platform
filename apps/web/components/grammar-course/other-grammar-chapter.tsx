...
import type { SatGrammarEntry } from '@/lib/sat-grammar';
import { grammarBasePath } from './grammar-api';
import styles from './sat-grammar.module.css';

export function OtherGrammarChapter({
  entry,
  previous,
  next,
}: {
  ...
  return (
    <div className={styles.page}>
      <nav aria-label="面包屑" className={styles.breadcrumb}>
        ...
        <Link href={courseBase}>其他语法</Link>
        <ChevronRight size={13} />
        <span>{entry.label}</span>
      </nav>

      <header className={styles.chapterHeader}>
        <p className={styles.eyebrow}>Grammar Beyond SAT</p>
        <h1>
        ...
        <div className={styles.chapterHeaderActions}>
          <span>
            {ruleCount} 个非重复知识点 · {ruleCount} 组正误例句
          </span>
        </div>
      </header>
      ...
      <p className={styles.sourceNote}>
        主题框架依据《剑桥初级英语语法》《剑桥中级英语语法（第4版）》和《剑桥高级英语语法》的目录；已与
        SAT 语法课程逐项去重，讲解与例句为平台原创。
      </p>
